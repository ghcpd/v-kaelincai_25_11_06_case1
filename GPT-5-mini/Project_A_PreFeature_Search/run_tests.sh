@echo off
set LOGFILE=logs\log_pre.txt
set RESULTFILE=results\results_pre.json
if not exist logs mkdir logs
if not exist results mkdir results

echo Starting Project A server...
...
try:
    r = requests.get('http://127.0.0.1:8001/search', params={'q':'shirt'}, timeout=2).json()
