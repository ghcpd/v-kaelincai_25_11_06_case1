...
RESULTS_PATH = os.path.join(BASE, 'results', 'results_pre.json')
LOG_PATH = os.path.join(BASE, 'logs', 'log_pre.txt')
...
for case in cases:
    ...
    try:
        if isinstance(q, str):
            res = requests.get(f'http://127.0.0.1:5000/search', params={'q': q}, timeout=5)
            latency_ms = int((time.time()-start)*1000)
            data = res.json()
            returned_ids = [r['id'] for r in data.get('results', [])]
            # Pass if at least one expected ID is in top 3
            expected = case.get('expected', [])
            ok = any(e in returned_ids[:3] for e in expected) if expected else (returned_ids == [])
            results.append({'test_id': test_id, 'query': q, 'latency_ms': latency_ms, 'returned_ids': returned_ids, 'expected': expected, 'pass': ok})
        else:
...
print(f'Pre-feature tests: {passed}/{len(results)} passed')
