"""
Test suite for Project B - Post-Feature Enhanced Search
"""
...
class TestRunner:
    """Test runner for enhanced search functionality."""
    ...
    def __init__(self, test_data_file: str, products_file: str):
        ...
        self.search_engine = EnhancedSearchEngine(products_file)
        self.results = []
    ...
    def run_test_case(self, test_case: dict) -> dict:
        ...
        related_products = search_result.get('related_products', [])
        ...
        if expected_results.get('should_have_suggestions', False):
            if not suggestions:
                passed = False
                failure_reason = "Expected search suggestions but none were provided"
        
        if expected_results.get('should_have_recommendations', False):
            if not related_products:
                passed = False
                failure_reason = "Expected related product recommendations but none were provided"
        
        # Check relevance scoring (only if we have results)
        if expected_results.get('should_have_relevance_scores', True) and len(actual_results) > 0:
            if not relevance_scores:
                passed = False
                failure_reason = "Expected relevance scores but none were provided"
        ...
        # Check result ordering (top result should have highest relevance)
        if len(actual_results) > 1 and relevance_scores:
            top_score = relevance_scores.get(actual_results[0].get('id'), 0)
            second_score = relevance_scores.get(actual_results[1].get('id'), 0)
            if top_score < second_score:
                passed = False
                failure_reason = f"Results not properly sorted by relevance: top={top_score}, second={second_score}"
        ...
        return {
            ...
            'num_recommendations': len(related_products),
            'has_relevance_scores': bool(relevance_scores),
...
def main():
    ...
    results_file = os.path.normpath(os.path.join(base_dir, 'results', 'results_post.json'))
    ...
    # Print summary
    print("\n" + "="*60)
    print("PROJECT B - POST-FEATURE ENHANCED SEARCH TEST RESULTS")
    print("="*60)
    ...
    for result in results['test_results']:
        ...
        print(f"    Suggestions: {result['num_suggestions']}, "
              f"Recommendations: {result['num_recommendations']}, "
              f"Top Relevance: {result['top_relevance_score']:.2f}")
